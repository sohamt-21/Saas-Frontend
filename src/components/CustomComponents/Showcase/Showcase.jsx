...
function Showcase({Component1, Component2}) {
  ...
  return (
    <div className=" border-4">
      ...
      >
        <div
          className=" h-96 w-1/2 cursor-none flex items-center justify-center border"
      ...
      {showCursor && (
        <motion.div
          ...
          className=" h-[100px] w-[100px] rounded-full flex flex-col justify-center items-center pointer-events-none fixed bg-cyan-400 -translate-x-1/2 -translate-y-1/2 mix-blend-darken"
          initial={{ opacity: 0, scale: 0 }}
          animate={{ opacity: 1, scale: 1 }}
          exit={{ opacity: 0, scale: 0 }}
          transition={{ duration: 0.3 }}
        >
          <MoveUpRight size={30} style={{ marginLeft: "8px" }} />
          {cursorText}
        </motion.div>
      )}
    </div>
